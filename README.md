...
主要功能包括数据可视化、空气质量排行、PM2.5 预测、污染物预警、历史数据导出、API(注：未登陆用户仅可使用部分数据可视化与部分空气质量排行功能)、后台管理。
...
采用异步方式处理模型训练请求，Spring 端会将用户的训练请求发送至 Kafka，Python 端从 Kafka 中取出请求开始训练模型。
...
|URL/参数|含义|
|:--|:--|
|site|监测点，为拼音代号，如`jingan`、`hongkou`|
...
基于 Spring AOP 与反射实现 Controller 层的统一日志打印，只需要在打印日志的方法前加上自定义的注解 `@MyLogger` 即可，日志内容包括：
